...
// Track A 判卷: 功能(隐藏 TB) + QoR 红线(vivado-flow OOC synth)
// 用法: node grade-track-a.cjs --task <taskDir> --rtl <fileOrDir> --out <dir> [--skip-synth]
...
function main() {
  ...
  if (!taskDir || !rtlArg || !outDir) {
    console.error('Usage: node grade-track-a.cjs --task <taskDir> --rtl <fileOrDir> --out <dir> [--skip-synth]');
    process.exit(2);
  }

  const manifest = loadTask(taskDir);
  const track = manifest.tracks.A;
  const lockErrors = verifyLocks(taskDir, manifest);
  if (lockErrors.length) {
    writeJson(path.join(outDir, 'grade.json'), { task: manifest.id, track: 'A', verdict: 'invalid_assets', lockErrors });
    console.error('RESULT: INVALID (asset locks)\n' + lockErrors.join('\n'));
    process.exit(2);
  }

  const rtlFiles = collectRtlSources(rtlArg);
  const grade = { task: manifest.id, track: 'A', rtl: rtlFiles, functional: null, qor: null, verdict: 'fail' };
  ...
  // 2) QoR 红线: OOC synth(功能挂了就不烧综合时间)
  if (grade.functional.pass && !skipSynth) {
    // vivado-flow 按目录收源文件;单文件交付时先归拢到 synth_src
    let srcDir;
    ...
    grade.qor = { skipped: true, pass: true };
  }

  grade.verdict = grade.functional.pass && (grade.qor ? grade.qor.pass : false) ? 'pass' : 'fail';
