...
function publicVerifier() {
  return [
    ...
    '}',
    "requirePattern('held branch must output stored r_data', /if\\s*\\(\\s*r_hold\\s*\\)\\s*begin[\\s\\S]{0,260}ro_m_data\\s*<=\\s*r_data/);",
    ...
    "if (!/ri_m_ready\\s*=\\s*1'b0|ri_m_ready\\s*=\\s*0/.test(tbCode) || !/ri_m_ready\\s*=\\s*1'b1|ri_m_ready\\s*=\\s*1/.test(tbCode)) {",
...
function buildPrompt(previous = null) {
  ...
  return [
    ...
    '    "checkpoint2": "coding self-check for ri_/ro_, sync reset, no latch, bit widths"',
    '  },',
    '  "actions": [',
    '    {',
    '      "type": "write_file",',
    ...
    '- All ro_ outputs must be registered from always_ff @(posedge i_clk).',
...
function compliantTb() {
  return [
    ...
    '    begin',
    '      if (!ro_m_valid || ro_m_data !== expected) begin',
...
function cannedPlan() {
  return {
    ...
    checkpointEvidence: {
      ...
      checkpoint2: 'Checked ri_/ro_ ports, synchronous reset, sequential ro_ assignments, no latch paths, and P_DATA_W widths.',
    },
    actions: [
      {
        type: 'write_file',
