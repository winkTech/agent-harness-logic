// ============================================================================
// UVM Top Level: Synchronization (包检测 + 定时 + CFO 校正)
// 额外监测: fft_start, sync_locked 控制信号
// ============================================================================
...
module tb_sync_uvm_top;
    ...
    // 控制信号监测 (暂不接入 UVM config_db, 留待扩展)
    logic fft_start;
    logic sync_locked;

    sync_top #(.DATA_W(16)) u_dut (
        .clk            (clk),
        .rst_n          (rst_n),
    ...
    initial begin
        run_test("sync_basic_test");
    end

    initial begin
        $dumpfile("tb_sync_uvm_top.vcd");
        $dumpvars(0, tb_sync_uvm_top);
    end
