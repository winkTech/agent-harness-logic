...
Agent解读PDF必须按顺序进行，中间遇到图表进行解读与理解后，再往后进行。

### 转换优先原则

读取PDF前，先使用markdown-converter将PDF转成MD文件，再进行图文阅读与分析、理解。

### 图文结合原则

图表解读必须结合上下文文本，理解图表在文档中的作用和含义。
...
def pdf_to_markdown_ocr(pdf_path, md_path, lang='chi_sim+eng'):
    doc = fitz.open(pdf_path)
    with open(md_path, 'w', encoding='utf-8') as f:
        for i, page in enumerate(doc):
...
def auto_convert_pdf(pdf_path, md_path):
    """自动选择最佳转换方法：markitdown > pymupdf > OCR"""
    # 方法1：尝试使用markitdown-converter（工具位于 rag-skill/tools/cli/）
    try:
        import subprocess
        ...
        if result.returncode == 0 and os.path.exists(md_path):
            return {'success': True, 'method': 'markitdown'}
    except Exception:
        pass
    ...
    try:
        ...
        if not has_text:
            return pdf_to_markdown_ocr(pdf_path, md_path)
    except Exception:
        pass
        ...
        with open(md_path, 'w', encoding='utf-8') as f:
            for i, page in enumerate(doc):
                ...
                if text.strip():
                    ...
                    f.write('\n\n')
        doc.close()
        return {'success': True, 'method': 'pymupdf'}
