...
| **1** 架构设计 | 算法文档化 + 顶层框图 (模块↔MATLAB 对标) + 每模块方案 + architecture.yaml | algorithm_spec + 框图 + golden model 通过 |
...
| **4** 逐模块RTL+MATLAB验证 | **[MUST]** 每模块: 写RTL → 仿真 → 对比MATLAB → 通过 → 下一模块 | 验证矩阵全 ✅，每模块 bit-true 报告 |
