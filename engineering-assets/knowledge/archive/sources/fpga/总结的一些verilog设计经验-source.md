...
| 规则     | 在一个always块中同时有组合和时序逻辑时时，采用非阻塞赋值语句。 |     |
#4:
| 规则  | 不要在一个always块中同时采用阻塞和非阻塞赋值语句。 |     |
| --- | ---------------------------- | --- |
#5:
...
例如 A 是INTEGER型，范围从0到255；B是STD_LOGIC_VECTOR，定义为8位。A累
加到255
时，
再加1就一直保持255不变，不会自动反转到0，除非令其为0；而B累加到255时，再加
1就
会自动反转到0。所以在使用时要特别注意！
以触发器为例说明描述的规范性
1，无置位/清零的时序逻辑
...
进行仿真测试：
1，避免使用内部生成的时钟
内部生成的时钟称为门生时钟（gatedclock）。如果外部输入时钟和门生时钟同
时驱动，
