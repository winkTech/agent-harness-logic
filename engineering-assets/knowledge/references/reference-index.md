...
| `skills/workflows/architecture-review-workflow.md` | `skills/workflows/architecture-review/` (4 个 Phase) | 多 Agent 架构审查：上下文→分析→安全→建议 |
...
| `engine/scripts/archive/agent-context-watchdog.cjs` | Agent spawn 追踪 + 自压缩指令注入 + context 统计报表 |
| `engine/scripts/archive/ctx-checkpoint.sh` | PreCompact hook 调用的压缩前 checkpoint |
...
# 查 agent 上下文健康摘要
node engine/scripts/archive/agent-context-watchdog.cjs health

# 记录 agent spawn
node engine/scripts/archive/agent-context-watchdog.cjs track planner

# 查看所有已 spawn agent
node engine/scripts/archive/agent-context-watchdog.cjs status
```
