...
——本版纳入 `run_xsim.sh` 后首跑即暴露它是坏的，且坏得看不出来：

### 1. 与另外两个 TB 同一个坑：失败读作通过

判决写的是 `$finish(errors ? 1 : 0)`，超时路径写的是 `$finish(1)`。
`$finish(N)` 的 N 是诊断详略等级、不是退出码——**跑飞了也以 0 退出**。
本包 README 早在 2026-07-28 就点名记过这个写法，当时只修了编码器 TB；
译码器 TB 于 1.0.2 补修；本全链路 TB 到现在才跟上。**同一个坑在三个 TB 里各躺了一份。**

### 2. 驱动循环不按 AXI 握手推进

四个 driver/collector 任务都是 `for(i..){ @(posedge clk); tdata <= x[i]; while(!ready) @(posedge clk); }`
——索引每轮无条件自增，**不确认本拍是否真的成交**。编码器 2026-07-31 修掉
"`S_IDLE` 就拉 ready 吃掉首位"之后 ready 只在 `S_LOAD` 有效，这个驱动便与 DUT 失步，
实测在**第 1/10 个测试就超时**。

这与 `rrc_polyphase_fir` 0.4.0 记过的"固定节拍驱动、不看 tready ⇒ 静默丢符号"
是同一类缺陷。四个任务全部改为只在 `tvalid && tready` 成交那一拍推进，写法照抄
同包 `tb_ldpc_encoder_top.v` 的 `drive_info`。

### 3. 看门狗额度从一开始就不够

改好握手后 9/10 通过、第 10 个撞上看门狗。查明：单次编解码实测约 22000 拍，
10 次约 220000 拍，而看门狗写死 200000 拍——**本来就不够**。此前握手缺陷让它在
第 1 个测试就超时，把这一层掩盖住了。改为按 `N_TESTS` 定额
（`N_TESTS * 25000 * 2`，单次含裕量再留一倍余量）。

### 结果

**10/10 全链路 bit-exact PASS**（noise-free BPSK，encoder → decoder 环回，
324 信息位逐位比对）。RTL、约束零改动；证据口径不变（G-B-03 仍由译码器 TB 承担，
本 TB 是补充覆盖）。

## [1.0.4] — 2026-08-02 复位检查采样时刻更正（RTL 零改动）

1.0.2 留下的未决项已按 owner 裁定处理：`run_reset_check` 的采样时刻由
**复位释放后 +1 拍**改为**复位仍断言期间**。

原写法在 deassert 后再等一个 posedge 才采样，那时寄存器已经执行过一次非复位
分支、装载的是**功能次态而非复位值**。对多数寄存器两者碰巧相同（次态也是 0），
于是长期没被发现；迁 xsim 时暴露：`cn_update.ro_lr` 在那一拍装载 `w_lr`，
而 `w_lr` 溯源到未初始化的 `msg_buffer`——xsim 给 X、ModelSim 给 0，
26 个寄存器里只有它由存储器阵列喂。
...
改后实测：**26/26 全过**，10 组 3240 bit 0 失配，`BIT-TRUE PASS`。

途中踩到一个自己造的坑并修掉：采样点前移后，原本紧跟其后的去断言语句被一并
删掉，导致复位永不释放、后续向量全部超时（向量 1 只收到 0/324 位）。已在采样
完成后显式补上去断言。

`reset-sim.json` 的 `method` 字段同步更新，并加 `method_note` 说明变更原委。
