...
| ID | 判据 | 如何检查 | 通过条件 | 证据产物 | 硬门 | 级别 |
|:--|:--|:--|:--|:--|:--|:--|
| G-C-01 | 目标 fmax 收敛（**紧时钟交叉核对**） | tool | 吸收 checkability（堵"松时钟白 WNS"后门）：解析 `constraints.target.fmax`（字符串带单位，如 `250MHz`）为 `fmax_mhz`，`required_period_ns = 1000/fmax_mhz`；SDC/XDC `create_clock` 的 `period_ns ≤ required_period_ns`（约束不松于目标）；WNS≥0 只有在此时钟下才计。约束缺失或周期宽于目标 ⇒ FAIL。MVP=Vivado synth 级时序估计。**已接线**（2026-07-25）：证据由 `tools/pg-synth.cjs` 生成，判据由 `gate-runner.cjs` 解析 `timing-summary.rpt` 的 Clock Summary（周期）与 Intra-Clock Table（该时钟 WNS）。另设两道防顶账：报告周期须与 XDC 约束一致，否则 FAIL；找不到该时钟的 WNS 行则标 blocked 而非 pass。`manifest.device.part` 必须声明——WNS 只对特定 part+speed grade 成立 | `timing-summary.rpt` + `envelope-check.json`（constrained vs required） | MUST（无 EDA 则封顶 qualification） | certified |
| G-C-02 | 资源在包络内（**fail-closed**） | tool | 吸收 checkability（堵"空包络静默通过"）：`constraints.target` 缺任一 `lut/ff/bram/dsp` 数值预算 ⇒ 标 **blocked**（不可在无包络下认证资源稳定），非 pass；有预算则实测 ≤ 预算逐项比对。**已接线**（2026-07-25）：从 `utilization.rpt` 解析 Slice LUTs / Slice Registers / Block RAM Tile / DSPs 四项实测值逐项比对；报告中缺某项行时该项标 blocked，不按 0 处理 | `utilization.rpt` + `envelope-check.json`（每资源 budget/achieved/status） | MUST | certified |
| G-C-03 | 无仿真-综合差异 | hybrid | **按判据而非字面计数**（2026-07-25 精化）：本门要防的是"综合器忽略 `initial` 而仿真执行它"导致的上板行为差异。故区分两类：(a) `initial` 对**标量/向量寄存器**赋初值 ⇒ **FAIL**（真差异源）；(b) `initial` **仅**对存储器阵列赋值 ⇒ 不判 fail，标 **blocked** 待综合报告证实（Xilinx 即以此推断 BRAM/LUTROM 初值，是标准可综合写法；原表述"ROM 用 `$readmemh`"自相矛盾——`$readmemh` 本就写在 `initial` 内）。判别依据：声明中标识符后紧跟 `[` 者为存储器阵列。TB 豁免。推断锁存器=0（引 G-A-05）。**blocked 不等于放行**：该级仍不予晋升，与 runner"tool 类门未接线标 blocked、绝不静默放行"一致。**裁决已接线**（2026-07-25）：blocked 由 Vivado 综合日志裁决——出现 `[Synth 8-6896]`（initial 块被综合器整体忽略）即 FAIL 并附 `[Synth 8-3848]`（网络无驱动源）作后果佐证；无该告警才转 pass。首次应用即在 `ldpc_codec/rtl/h_matrix_addr.v:137` 命中真实差异：连接索引表 `r_conn_col/r_conn_shft/r_conn_cnt` 因 `if` 条件非常量而被丢弃，仿真有值、上板无驱动 | `no-initial-scan.json` + `synth.log` | MUST | qualification |
...
| CS-2 | manifest = 单一真相源 | 过 `cbb-manifest.schema.json`（ajv）；`rtl/`+`tb/` 每文件 sha256==声明值；无未登记源文件。**哈希口径＝LF 归一内容**（2026-07-25 定）：git 在 Windows 检出 CRLF、Linux 检出 LF，同一 commit 的同一文件原始字节哈希随平台而变，按检出字节比对会让本门在 Windows 上整体误报（实测 92/127 声明值全部且仅因行尾差异不符）。归一只吸收行结束符，不掩盖任何实质内容改动 | machine | `manifest-hash-audit.json` | intake |
