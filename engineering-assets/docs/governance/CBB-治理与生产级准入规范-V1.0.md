...
| G-C-03 | 无仿真-综合差异 | hybrid | **按判据而非字面计数**（2026-07-25 精化）：本门要防的是"综合器忽略 `initial` 而仿真执行它"导致的上板行为差异。故区分两类：(a) `initial` 对**标量/向量寄存器**赋初值 ⇒ **FAIL**（真差异源）；(b) `initial` **仅**对存储器阵列赋值 ⇒ 不判 fail，标 **blocked** 待综合报告证实（Xilinx 即以此推断 BRAM/LUTROM 初值，是标准可综合写法；原表述"ROM 用 `$readmemh`"自相矛盾——`$readmemh` 本就写在 `initial` 内）。判别依据：声明中标识符后紧跟 `[` 者为存储器阵列。TB 豁免。推断锁存器=0（引 G-A-05）。**blocked 不等于放行**：该级仍不予晋升，与 runner"tool 类门未接线标 blocked、绝不静默放行"一致 | `no-initial-scan.json` | MUST | qualification |
